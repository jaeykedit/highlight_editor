        ...
        try {
            ...
            for (var i = 0; i < markers.numMarkers; i++) {
                ...
                try {
                    ...
                    var newMarker = seqMarkers.createMarker(startTime); // 초 단위 숫자로 먼저 시도
                    newMarker.name = comment;
                    newMarker.end.seconds = endTime; // 구간 마커로 설정
                    logFile.writeln(dateStr + ": Duration marker " + (markersProcessed + 1) + " created successfully on sequence at " + startTime + "s to " + endTime + "s");
                } catch (e) {
                    logFile.writeln(dateStr + ": Failed to create duration marker with seconds at " + startTime + "s: " + e.toString());
                    // 대체로 Time 객체 시도
                    try {
                        var time = new Time();
                        time.seconds = startTime;
                        ...
                        newMarker.name = comment;
                        newMarker.end.seconds = endTime; // 구간 마커로 설정
                        logFile.writeln(dateStr + ": Duration marker " + (markersProcessed + 1) + " created successfully using Time object at " + startTime + "s to " + endTime + "s");
                    } catch (e2) {
                        logFile.writeln(dateStr + ": Failed to create duration marker with Time object at " + startTime + "s: " + e2.toString());
                    }
                }
            ...
            alert("Success: Duration markers added to sequence, clips inserted on Video 2, and cuts attempted on Video 1. Check sequence markers and Video 1 cuts manually.");
            return "Success";
        } catch (e) {
